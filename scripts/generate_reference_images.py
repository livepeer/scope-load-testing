"""Generate reference images for i2v testing via the Daydream SDK inference API.

Uses the SDK /inference endpoint with flux-dev/schnell to generate
512x512 images, downloads them locally, and updates the manifest.
            ...
            try:
                with httpx.Client(timeout=httpx.Timeout(connect=30.0, read=120.0, write=30.0, pool=30.0)) as client:
                    resp = client.post(
                        ...
                        json={"capability": cap, "prompt": prompt, "width": 512, "height": 512},
                    )
                    if resp.status_code in TRANSIENT_STATUS_CODES:
                        logger.warning("  %d for %s (attempt %d/%d)", resp.status_code, cap, attempt + 1, retries)
                        time.sleep(5)
                        continue
                    ...
                    # Unexpected shape — treat as transient, try next attempt
                    logger.warning("  Unexpected response for %s: %s (attempt %d/%d)", cap, list(data.keys()), attempt + 1, retries)
                    time.sleep(2)
                    continue

            except httpx.HTTPStatusError as e:
                if e.response.status_code in TRANSIENT_STATUS_CODES:
                    logger.warning("  %d for %s (attempt %d/%d)", e.response.status_code, cap, attempt + 1, retries)
                    time.sleep(5)
                    continue
                raise
    return None
